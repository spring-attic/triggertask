...
NOTE: When using the tasklauncher-* and the URI uses maven as the resource
please be sure to set the `--maven.remote-repositories` property for the
tasklauncher-*, if the jar is not in your local repository.  For example:
